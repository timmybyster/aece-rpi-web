...
function preload() {
	...
	game.load.image('cbb', $('#cnfCbbImg').val());
	game.load.image('edd', $('#cnfEddImg').val());
...
function createNodeVisuals() {
    ...
    $(allNodeData).each(function(index, elem) {
		...
		// detonator status
		if (node['type'] === 'IB651' || node['type'] === 'CBB' ) {
			...
			if(node['type'] === 'CBB' ){
				detonatorStatusConnected.inputEnabled = true;
				detonatorStatusConnected.events.onInputDown.add(showDetonators,this);
				detonatorStatusNotConnected.inputEnabled = true;
				detonatorStatusNotConnected.events.onInputDown.add(showDetonators,this);
			}
		}
