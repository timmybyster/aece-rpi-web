...
function create(){
	...
	game.world.setBounds(-100, -100, 1000, 1000);
}
...
function createNodeVisual(node){
	...
	nodeVisual.events.onInputDown.add(focusCameraOnNode(node.x, node.y), this);
	
	nodeVisuals[node.id] = nodeVisual;
	console.log(nodeVisuals[node.id].db_id);
}
...
function focusCameraOnNode(x, y){
	game.camera.x += x;
    game.camera.y += y;
}
